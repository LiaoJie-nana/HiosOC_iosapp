...
- (void)setup {
    ...
    [self setImage:[UIImage imageNamed:@"like_before"] forState:UIControlStateNormal];
    [self setImage:[UIImage imageNamed:@"like_after"] forState:UIControlStateSelected];
...
- (void)startAnimate {
    ...
    {
        //144张图片 随机选15张
        int x = arc4random() % 144 + 1;
        NSString * imageStr = [NSString stringWithFormat:@"%d",x];
...
- (NSMutableAttributedString *)getAttributedString:(NSInteger)num
{
    ...
    if (num <= 3) {
        ...
        attch.image = [UIImage imageNamed:@"guli"];
    ...
    }else if (num <= 6)
    {
        //加油
        NSTextAttachment *attch = [[NSTextAttachment alloc] init];
        attch.image = [UIImage imageNamed:@"jiayou"];
        attch.bounds = CGRectMake(0, 0, attch.image.size.width, attch.image.size.height);
        NSAttributedString *z_string = [NSAttributedString attributedStringWithAttachment:attch];
        [mutStr appendAttributedString:z_string];
    }else
    {
        //太棒了
        NSTextAttachment *attch = [[NSTextAttachment alloc] init];
        attch.image = [UIImage imageNamed:@"taibangle"];
        attch.bounds = CGRectMake(0, 0, attch.image.size.width, attch.image.size.height);
        NSAttributedString *z_string = [NSAttributedString attributedStringWithAttachment:attch];
        [mutStr appendAttributedString:z_string];
    }
